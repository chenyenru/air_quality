df <- read.csv("DATA/more/merge2.csv")
df <- df[,-1]
df
View(df)
library(imputeTS)
...
year + county + waste_recycled_muni + bulk_waste_recycled + rate_material_recov + 
locomobiles_density + automobiles_density + factory_density + doctor + ozone + revenue + sex + cancer + deaths + cancer_counts + mean_age + median_age + raw_death_rates
