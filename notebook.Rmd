...
fixed.time <- plm(breast ~ waste_recycled_muni + automobiles_density + factory_density + hospital_bed + ozone + revenue + employee_industrial + family_income + so2 + pm + co + no2, data = pdf, model = "within")
summary(fixed.time)
...
```{r}
pool <- plm(breast ~ ozone, data = pdf, model = "pooling")
summary(pool)
```
