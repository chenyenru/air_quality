...
colnames(file)[12] <- "raw_death_rates"
file <- subset(file, year >= 2000)
...
```{r}
model<- (raw_death_rates ~ waste_recycled_muni + locomobiles_density + automobiles_density + hospital_bed + ozone + revenue + family_income + rate_material_recov + so2 + pm) 
...
fixed.time <- plm(raw_death_rates ~ waste_recycled_muni + locomobiles_density + automobiles_density + hospital_bed + ozone + revenue + employee_industrial + family_income + rate_material_recov + so2 + pm + factor(year), data = pdf, model = "within")
summary(fixed.time)
```
